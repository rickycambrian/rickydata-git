...
_SECRET_TOOLKIT_BASENAMES = {
    "secret-filter.py",
    "secret-guard.py",
    "scrub-claude-home.py",
    "setup-secret-scrub.sh",
    "audit-history-secrets.sh",
    "safe-history-rewrite.sh",
}
...
def _should_skip_file(file_path: str) -> bool:
    ...
    if "/build/" in file_path or file_path.startswith("build/"):
        return True
    # The secret-obfuscation toolkit's own implementation files necessarily
    # reference identifiers like `secret_map`; do not flag the scanner on itself.
    if file_path.rsplit("/", 1)[-1] in _SECRET_TOOLKIT_BASENAMES:
        return True
    return False
